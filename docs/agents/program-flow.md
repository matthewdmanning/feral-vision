...
For cloud-published Datasets, Cloud Build first writes the selected layout to a
versioned Cloud Storage prefix. A DVC-scoped repository then records the exact
object generations as the Dataset Artifact. The VM stages that pinned artifact
and its data from Cloud Storage; it does not resolve a moving bucket prefix.
...
| Tool | Owns |
|---|---|
| DVC | Raw, processed, and augmented Datasets and Dataset Artifacts, including the Git-tracked pointers to versioned cloud objects; never training/evaluation runs, checkpoints, or metrics. |
