...
The canonical training entrypoint is ``feral_segmentor.training.trainer``, wired
via Hydra:
...
This builds the model (:mod:`feral_segmentor.models.registry`), optimizer,
scheduler, and loss function from ``conf/train/`` (see :doc:`../api/training`),
then runs :meth:`~feral_segmentor.training.trainer.Trainer.fit`. Metrics are
logged to MLflow when a run is active, and the best checkpoint is written to
``models/registry/best.pt``.

.. note::

   ``feral_segmentor.training.train`` (a direct ``ultralytics.YOLO(...).train()``
